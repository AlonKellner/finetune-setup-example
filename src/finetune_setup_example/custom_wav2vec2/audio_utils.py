...
This code is pure numpy to support all frameworks
...
def load_audio(
    audio: str | np.ndarray, sampling_rate: int = 16000, timeout: float | None = None
) -> np.ndarray:
    """
    Load `audio` to an np.ndarray object.
...
def load_audio_as(
    ...
    try:
        ...
        # Process audio data
        with io.BytesIO(audio_bytes) as audio_file, sf.SoundFile(audio_file) as f:
        ...
        raise ValueError(f"Error loading audio: {e}") from e
...
def is_valid_audio(audio: list[AudioInput] | AudioInput) -> bool:
    """Check if the input is a valid audio format."""
    return is_numpy_array(audio) or is_torch_tensor(audio)


def is_valid_list_of_audio(audio: list[AudioInput] | AudioInput) -> bool:
    """Check if the input is a valid list of audio formats."""
    return (
        (audio is not None)
        and (len(audio) > 0)
        and all(is_valid_audio(audio_i) for audio_i in audio)
    )
...
def hertz_to_mel(
    freq: np.ndarray | float, mel_scale: str = "htk"
) -> np.ndarray | float:
    """
    ...
    if isinstance(freq, np.ndarray) and isinstance(mels, np.ndarray):
...
def mel_to_hertz(mels: np.ndarray, mel_scale: str = "htk") -> np.ndarray:
    """
...
def chroma_filter_bank(
    ...
    """
    Create a chroma filter bank, i.e a linear transformation to project spectrogram bins onto chroma bins.
...
def mel_filter_bank(
    ...
    """
    Create a frequency bin conversion matrix used to obtain a mel spectrogram.

    This is called a *mel filter bank*, and
...
def optimal_fft_length(window_length: int) -> int:
    """
    Find the best FFT input size for a given `window_length`.
...
def window_function(
    ...
    """
    Return an array containing the specified window. This window is intended to be used with `stft`.
...
def fast_spectrogram(  # noqa: PLR0912, PLR0915
    waveform: np.ndarray,
    window: np.ndarray,
    frame_length: int,
    hop_length: int,
    ...
    """
    Calculate a spectrogram over one waveform using the Short-Time Fourier Transform.
    ...
    Returns
    -------
        `and.array` containing a spectrogram of shape `(num_frequency_bins, length)` for a regular spectrogram or shape
    ...
    # 1. Create a view of the waveform in overlapping frames
    # This is the modern and safe way to create sliding windows.
    frames = np.lib.stride_tricks.sliding_window_view(
        waveform, window_shape=frame_length
    )[::hop_length]

    # Make a copy to modify the data (since frames is a view)
    frames = frames.copy()

    # 2. Apply preprocessing to all frames at once (vectorized)
    if dither != 0.0:
    ...
    # 3. Apply the window function to all frames
    frames *= window

    # 4. Perform the FFT on all frames
    # We need to pad with zeros if fft_length > frame_length
    if fft_length > frame_length:
        pad_width = fft_length - frame_length
    ...
    if power is not None and log_mel is not None:
        ...
        spectrogram = orig_np.asarray(spectrogram, dtype)
    return spectrogram


def fast_spectrogram_batch(  # noqa: PLR0912, PLR0915
    ...
    """
    Calculate spectrograms for a list of waveforms using the Short-Time Fourier Transform, optimized for batch processing.
    ...
    """
    waveform_list = [
        (w if isinstance(w, np.ndarray) else np.asarray(w)) for w in waveform_list
    ]
    window_length = len(window)
    ...
    # This calculation for the true number of frames is already efficient
    ...
    # 1. Frame the entire batch of waveforms at once 🖼️
    # Input shape: (batch_size, num_samples)
    # sliding_window_view creates a view of shape: (batch_size, num_frames, frame_length)
    # The [:, ::hop_length] slice selects the frames with the correct hop.
    frames = np.lib.stride_tricks.sliding_window_view(
        padded_waveform_batch, window_shape=(frame_length,), axis=-1
    )[:, ::hop_length, :]

    # Make a copy to enable in-place modifications
    frames = frames.copy()

    # 2. Apply preprocessing to the entire 3D frames array 🛠️
    # Operations are applied along the last axis (the frame_length dimension).
    # The ellipsis (...) selects all preceding dimensions (batch_size, num_frames).
    if dither != 0.0:
        frames += dither * np.random.randn(*frames.shape)
    ...
    # 3. Apply the window to all frames in the batch 🪟
    # NumPy's broadcasting automatically applies the 1D window to the last axis
    # of the 3D frames array.
    frames *= window

    # 4. Perform FFT on all frames in the batch ⚡
    # The FFT is applied along the last axis of the frames array.
    # The 'n' parameter in rfft handles padding or truncating to fft_length.
    fft_func = np.fft.rfft if onesided else np.fft.fft
    spectrogram = fft_func(frames, n=fft_length, axis=-1)
...
# TODO This method does not support batching yet as we are mainly focused on inference.
def spectrogram(  # noqa: PLR0912, PLR0915
    waveform: np.ndarray,
    window: np.ndarray,
    frame_length: int,
    hop_length: int,
...
def spectrogram_batch(  # noqa: PLR0912, PLR0915
    waveform_list: list[np.ndarray],
    window: np.ndarray,
    frame_length: int,
    hop_length: int,
...
def power_to_db(
    ...
    """
    Convert a power spectrogram to the decibel scale.

    This computes `10 * log10(spectrogram / reference)`, using basic
...
def power_to_db_batch(
    ...
    """
    Convert a batch of power spectrograms to the decibel scale.

    This computes `10 * log10(spectrogram / reference)`,
...
def amplitude_to_db(
    ...
    """
    Convert an amplitude spectrogram to the decibel scale.

    This computes `20 * log10(spectrogram / reference)`, using
...
def amplitude_to_db_batch(
    ...
    """
    Convert a batch of amplitude spectrograms to the decibel scale.

    This computes `20 * log10(spectrogram / reference)`,
    using basic logarithm properties for numerical stability.
